...
val Sepia = Transformation { image ->
    require(image[0][0].size == 3) {"Sepia transformation can only be applied to RGB images."}
    ...
    for (i in 0 until height) {
        for (j in 0 until width) {
            val red = image[i][j][0].toDouble()
            val green = image[i][j][1].toDouble()
            val blue = image[i][j][2].toDouble()
